...
const MovieCard: React.FC<IMovieCard> = ({
  ...
  return (
    <div className="flex item-center justify-center">
      <div className="mx-auto bg-[var(--card)] rounded-3xl shadow-lg transform transition-transform duration-300 ease-out hover:scale-105 active:scale-95 group">
        <div className="grid rounded-3xl overflow-hidden">
          ...
          <div className="p-5 z-10">
            {/* Movie title */}
            <p className="text-lg font-bold text-[var(--foreground)] truncate">{title}</p>
            <p className="text-sm text-[var(--secondary-foreground)]">({releaseYear})</p>

            {/* Movie Description */}
            <div className="h-20">
              <span className="mt-2 text-[var(--foreground)] line-clamp-3">
                {description}
              </span>
            </div>
